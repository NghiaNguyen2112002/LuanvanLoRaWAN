...
void SR_Init(uint8_t RX, uint8_t TX, uint32_t baudrate){
    ...
    sensorSerial.begin((unsigned int)baudrate, SERIAL_8N1, RX, TX);
    delay(1000);
...
float SR_GetTemp(void){
    ...
    sensorSerial.write(cmd_temp, 8);

    timeOut = 2000 / 50;
    while( (timeOut--) && (!sensorSerial.available()) ){
    ...
    if(sensorSerial.available()){
        ...
        sensorSerial.readBytes(receivedData, sizeof(receivedData));
...
uint16_t SR_GetPM2_5(void){
    ...
    sensorSerial.write(cmd_pm2_5, 8);
...
void SR_GetTempHumi(float* temp, float* humi){
    ...
    if(sensorSerial.available()){
        sensorSerial.readString();   
    }

    sensorSerial.write(cmd_temp_humi, 8);
...
void SR_GetSoil_PH(float* pH){
 int result;
    ...
    sensorSerial.write(cmd_soil_PH, 8);
...
void SR_GetSoil_TempHumi(float* sTemp, float* sHumi){
    ...
    sensorSerial.write(cmd_soil_temp_humi, 8);
...
void SR_GetSoil_NPK(uint16_t* nito, uint16_t* phoptpho, uint16_t* kali){
    ...
    sensorSerial.write(cmd_soil_npk, 8);
...
void SR_GetSoil_Conductivity(uint16_t* conductivity){
    ...
    sensorSerial.write(cmd_soil_conductivity, 8);
